...
class ProcessRecordController extends Controller
{
    public function index(Request $request, Process $process)
    {
        ...
        foreach ($tagNames as $tagName) {
            if (isset($validatedData[$tagName])) {
                ...
                $query->whereHas('processRecordTagValues.tag', function ($query) use ($tagName, $tagValues) {
        ...
        return ProcessRecordResource::collection($query->latest()->paginate());
    }

    public function show(Process $process, ProcessRecord $record)
    {
        return new ProcessRecordResource($record);
    }
    ...
    public function update(Request $request, Process $process, ProcessRecord $record) {
    ...
    public function destroy(Process $process, ProcessRecord $record)
    {
        $record->delete();
